...
class BatchSampler(Sampler):
    ...
    # path['rewards'] = np.swapaxes(path['rewards'], 0, 1)
    # path['actions'] = np.swapaxes(path['actions'], 0, 1)
    # path['observations'] = np.swapaxes(path['observations'], 0, 1)

    def cgmprocess_samples(self, itr, paths):
        baselines = []
        returns = []
        for path in paths:
            path_baselines = np.swapaxes(self.algo.baseline.predict(path),0,1)
            ...
            accumulatedReward = immediateReward * actionsTaken / sampleS[:, :, np.newaxis]
            temp = accumulatedReward[H - 1].sum(axis=1)
            for t in xrange(H - 2, -1, -1):
                accumulatedReward[t] += (temp[np.newaxis, np.newaxis, :] * sampleA[t]).sum(axis=2) / sampleS[t, :,
                                                                                                     np.newaxis]
                temp = accumulatedReward[t].sum(axis=1)
            sampleS[zeroIndices] = 0
            path["returns"] = accumulatedReward.sum(axis=2)#special.discount_cumsum(path["rewards"], self.algo.discount)
            path["advantages"] = accumulatedReward - path["returns"][:,:,np.newaxis]#path_baselines
...
class CGMBatchPolopt(RLAlgorithm):
    """
    ...
    def train(self):
        ...
        for itr in xrange(self.current_itr, self.n_itr):
            with logger.prefix('itr #%d | ' % itr):
                ...
                samples_data = self.sampler.cgmprocess_samples(itr, paths)
